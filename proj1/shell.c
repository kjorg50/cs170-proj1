...
{ 
  ...
  while( fgets(buff, sizeof(buff), stdin) != NULL )
	...
	if(child_p < 0){
	  printf("ERROR: inside of child process");
	  exit(1);
	} else if (child_p==0){
...
int cd(char * arg, int count){
  ...
  // if it is just a 'cd' command only, then go home
  if(count == 1){
    err1 = chdir(getenv("HOME"));
    if(err1!=0) printf("ERROR: could not change directory\n");
  } 
  else { // otherwise, go to the correct directory
    if(str[0]=='~'){
      if(str[1]=='/'){
	  char * path = &str[2];
	  //printf("*** after ~: %s\n",path);
	  err1 = chdir(getenv("HOME"));
	  err2 = chdir(path);
	  if(err1!=0 || err2!=0) printf("ERROR: could not change directory\n");
      }
      else if(str[1]=='\0'){ // this is the case of 'cd ~'
	err1 = chdir(getenv("HOME"));
	if(err1!=0) printf("ERROR: could not change directory\n");
      }
    }
    else{ // it is a directory that doesn't involve '~'
      err1=chdir(str);
      if(err1!=0) printf("ERROR: could not change directory\n");
    }
  }
